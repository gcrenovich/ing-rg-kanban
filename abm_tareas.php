...
// 1) ALTA DE TAREA (si POST trae 'titulo' y NO trae 'editar_id')
...
// 2) BAJA DE TAREA (si llega ?eliminar=ID)
if (isset($_GET['eliminar'])) {
  $stmt = $conn->prepare("DELETE FROM tareas WHERE id = ?");
  $stmt->execute([$_GET['eliminar']]);
}

// 3) EDICIÓN DE TAREA (si POST trae 'editar_id')
...
// 4) OBTENER DATOS PARA TABLA Y MODALES
$tareas   = $conn->query("
...
<body class="p-4">
  ...
  <table class="table table-bordered table-hover">
    <thead class="table-dark">
      <tr>
        <th>Título</th><th>Usuario</th><th>Urgencia</th><th>Estado</th>
        <th>Sector</th><th>Inicio</th><th>Fin</th><th>Diaria</th><th>Acciones</th>
      </tr>
    </thead>
    <tbody>
  ...
  <!-- 4) MODALES DE EDICIÓN (colocados FUERA de la <table>) -->
  <?php foreach ($tareas as $t): ?>
        ...
        <form method="POST" class="modal-content">
          ...
          <div class="modal-body">
            ...
            <input
              ...
              name="descripcion"
              class="form-control"
              value="<?= htmlspecialchars($t['descripcion']) ?>"
            >
